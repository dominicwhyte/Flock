...
class MapFirebaseClient: NSObject
{
    ...
    class func addEvent(_ eventName : String, eventStart : Date, eventEnd : Date, eventLocation : CLLocationCoordinate2D, eventType : EventType, eventImageURL : String?, eventDescription : String?, eventOwner : String?, eventPrivacy : String,  completion: @escaping (Bool) -> Void)
    {
    ...
    class func addEventReturnID(_ eventName : String, eventStart : Date, eventEnd : Date, eventLocation : CLLocationCoordinate2D, eventType : EventType, eventImageURL : String?, eventDescription : String?, eventOwner : String?, eventPrivacy : String, completion: @escaping (String) -> Void)
    {
        let eventID = FirebaseClient.dataRef.child("Active_Events").childByAutoId().key
        let updates = ["EventName": eventName as AnyObject, "EventID" : eventID as AnyObject, "EventStart" : DateUtilities.getStringFromFullDate(date: eventStart) as AnyObject, "EventEnd" : DateUtilities.getStringFromFullDate(date: eventEnd) as AnyObject, "Latitude" : eventLocation.latitude.description as AnyObject, "Longitude" : eventLocation.longitude.description as AnyObject, "EventType" : eventType.rawValue as AnyObject, "EventDescription": eventDescription as AnyObject, "EventOwner": eventOwner as AnyObject, "EventPrivacy": eventPrivacy as AnyObject] as [String : AnyObject]
        dataRef.child("Active_Events").child(eventID).updateChildValues(updates)
    ...
    static func createTestPins() {
        ...
        addEvent("Terrace Rager", eventStart: Date(), eventEnd: Date(), eventLocation: terracePosition, eventType: EventType.party, eventImageURL: nil, eventDescription: "I'm a Terrace event.", eventOwner: "Terrace Club", eventPrivacy: "Public", completion: { (success) in
            addEvent("Body Hype", eventStart: Date(), eventEnd: Date(), eventLocation: hypePosition, eventType: EventType.party, eventImageURL: nil, eventDescription: "I'm a chill Body Hype show.", eventOwner: "Body Hype Dance Co", eventPrivacy: "Public", completion: { (success) in
                addEvent("Cloister Rager", eventStart: Date(), eventEnd: Date(), eventLocation: cloisterPosition, eventType: EventType.party, eventImageURL: nil, eventDescription: "I'm a Cloister event.", eventOwner: "Cloister Club", eventPrivacy: "Public", completion: { (success) in
